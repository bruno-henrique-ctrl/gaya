...
const CollectorDashboard = () => {
  ...
  const fetchCollections = useCallback(async () => {
    ...
    if (auth.user.tipo !== "coletador") {
      setError(
        "Acesso negado. Apenas coletadores podem acessar este dashboard.",
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Usar direto os itens do backend
      const collections: GroupedCollection[] = res.data.map((col: any) => ({
        id: col.id,
        status: col.status,
        data: col.data,
  ...
  }, [rescheduleId, newDate, fetchCollections]);
  ...
  const openRescheduleDialog = useCallback(
    (id: number, currentDate: string) => {
      setRescheduleId(id);
      setNewDate(currentDate.slice(0, 16));
    },
    [],
  );
    ...
    return (
      <Container sx={{ mt: 5, textAlign: "center" }}>
        <Typography variant="h6">
          Você não possui coletas no momento.
        </Typography>
      </Container>
    );

  const statusLabels: Record<GroupedCollection["status"], string> = {
    pendente: "Pendente",
    agendada: "Remarcado",
    concluida: "Realizado",
    cancelada: "Cancelado",
  ...
  return (
    <Container sx={{ mt: 5 }}>
      ...
      <List>
        {groupedCollections
          .filter(
            (col) => col.status !== "concluida" && col.status !== "cancelada",
          )
          .map((col) => (
            <Paper
              key={col.id}
            ...
            >
              <ListItem disableGutters>
                <ListItemText
                  primary={`${new Date(col.data).toLocaleDateString()} | ${new Date(col.data).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} — ${formatEndereco(col.endereco)}`}
                  secondary={
                    <>
                      ...
                      {col.itens.map((m, index) => (
                        <Typography
                          ...
                          display="block"
                        >
                          Material: {m.tipo_material} — Quantidade:{" "}
                          {m.quantidade}
                        </Typography>
                      ))}
              ...
              <Stack direction="row" spacing={2} sx={{ mt: 1, ml: 1 }}>
                <Button
                  variant="text"
                  color="primary"
                  onClick={() => openRescheduleDialog(col.id, col.data)}
                >
                  Remarcar
                </Button>
                <Button
